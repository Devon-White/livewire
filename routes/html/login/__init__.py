...
from utils.session_utils import get_session_vars, set_subscriber_login, clear_subscriber_login, get_rest_client
...
def login():
    ...
    if not (project_id and auth_token and space_name):
        flash('SignalWire credentials missing from session. Please provide your credentials on the homepage.')
                ...
                if not subscriber_id:
                    ...
                    try:
                        ...
                        if address:
                            ...
                            logger.info(f"Marked subscriber {subscriber_id} as active with address {address}")
                        else:
                            logger.warning(f"Could not fetch address for subscriber {subscriber_id}")
